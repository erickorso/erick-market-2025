/**
 * Shared helpers for Vercel serverless under /api only.
 * Do NOT import from ../server — that crashes FUNCTION_INVOCATION_FAILED.
 */
...
export function filterWatchlist(q: string, category: CategoryId): WatchItem[] {
...
/** Bound concurrency to stay under Finnhub free-tier / Vercel time limits. */
...
export function mapStock(row: QuoteRow) {
  return {
    symbol: row.symbol,
    company: row.company,
    name: row.company,
...
export function isDayMovers(category: CategoryId) {
